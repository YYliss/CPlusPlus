...
void BitConvert::GetBytes(unsigned char* dest, float value)
{
	GetBytes(dest, *(int*)&value);
}

void BitConvert::GetBytes(unsigned char* dest, double value)
{
	GetBytes(dest, *(long long*)&value);
}
...
float BitConvert::ToFloat(unsigned char* data)
{
	int v = ToInt(data);
	return *(float*)&v;
}
double BitConvert::ToDouble(unsigned char* data)
{
	long long v = ToLLong(data);
	return *(long long*)&v;
}
...
void BitConvert::Test()
{
	...
	cout << "float 类型测试：：：：：：：：：：：：：：：：：：：：：：：：：：" << endl;
	unsigned char floatData[4];
	GetBytes(floatData, 10.055);
	cout << "打印 10.055 的转换结果 :" << ToFloat(floatData) << endl;


	cout << "double 类型测试：：：：：：：：：：：：：：：：：：：：：：：：：：" << endl;
	unsigned char DoubleData[4];
	GetBytes(floatData, 10.055);
	cout << "打印 10.055 的转换结果 :" << ToDouble(DoubleData) << endl;
}